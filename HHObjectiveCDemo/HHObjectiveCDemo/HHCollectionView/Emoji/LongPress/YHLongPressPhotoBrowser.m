...
/// 大图弹窗的父视图
@property (nonatomic, strong) UIView *showView_superView;
...
- (void)locationInTargetView:(UIView *)targetView
    ...
    CGRect targetRect = [self.showView_superView convertRect:targetView.frame fromView:targetView.superview];
    //计算宽高
    if (self.exWidth == 0) {
...
#pragma mark - 显示隐藏大图弹窗
- (void)showDisplayView:(CGFloat)display_x
              display_y:(CGFloat)display_y {
    if (!self.dispalyView.superview) {
        [self.showView_superView addSubview:self.showView];
        [self.showView_superView bringSubviewToFront:self.showView];
    }
...
- (UIView *)showView_superView {
    if (!_showView_superView) {
        self.showView_superView = [UIApplication sharedApplication].windows.firstObject;
    }
    return _showView_superView;
}
