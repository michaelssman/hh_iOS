...
#pragma mark - 解码完成的回调函数 设置session的时候的回调方法
/// (decompressionOutputCallback)，指向帧解压完成后的回调方法。你需要提供可以找到这个回调方法的实例 (decompressionOutputRefCon)。VTDecompressionOutputCallback 回调方法包括七个参数：
...
/// @param imageBuffer 实际图像的缓存
/// @param presentationTimeStamp 出现的时间戳
/// @param presentationDuration 出现的持续时间
...
- (BOOL)initDecoder {
    ...
    /**
     1.根据sps pps设置解码参数
     param kCFAllocatorDefault 内存分配器
     param 2 参数个数 sps pps
    ...
    * kCVPixelBufferWidthKey/kCVPixelBufferHeightKey: 视频源的分辨率 width*height
     * kCVPixelBufferOpenGLCompatibilityKey : 它允许在 OpenGL 的上下文中直接绘制解码后的图像，而不是从数据总线和 CPU 之间复制数据。这有时候被称为零拷贝通道，因为在绘制过程中没有解码的图像被拷贝.
...
#pragma mark - 解码函数（private）
- (CVPixelBufferRef)decode:(uint8_t *)frame withSize:(uint32_t)frameSize {
    
    ///先创建两个buffer
    ///CVPixelBufferRef解码后的数据，或者未编码数据
    ///CMBlockBufferRef编码后的数据
    ...
    //创建blockBuffer
    /*!
     参数1: structureAllocator 默认内存分配kCFAllocatorDefault
...
- (void)decodeNaluData:(uint8_t *)frame size:(uint32_t)size {
    ...
    switch (type) {
        case 0x05: //关键帧
            ...
            break;
        case 0x06:
            //NSLog(@"SEI");//增强信息，什么也不做
            break;
        case 0x07: //sps
            _spsSize = naluSize;
            _sps = malloc(_spsSize);//开辟空间
            memcpy(_sps, &frame[4], _spsSize);//把数据从&frame[4]位置开始拷贝到_sps
            break;
        case 0x08: //pps
            _ppsSize = naluSize;
